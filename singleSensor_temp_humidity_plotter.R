...
#setwd("/home/mayko/Carrboro_Recybery/kode/environmental_monitoring")
...
levels(single_sensor_test.summarized.gathered$Measure) <- c("Mean Relative Humidity (%)","Mean Temperature (C)")
png(filename="single_DHT11_timeline.png", width=1000, height=600)
ggplot(single_sensor_test.summarized.gathered  ) + geom_line(aes(x=meanTime, y=Value)) + facet_grid(Measure~., scale="free")  + geom_smooth(aes(x=meanTime, y=Value, color=Measure)) + theme(legend.position="none")+ ggtitle("DHT11 Trace, bedroom") + labs(x='Time', y='Measurement')
dev.off()
